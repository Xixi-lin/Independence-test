POWER_CALCULATIONd <- function(model, n , sd0, coef)
{
  library(HHG)
  library(minerva) 
  library(energy)
  ...
  BB = foreach(iter = 1:ITER, .combine = "rbind" ) %dopar% { # .packages = c("HHG", "minerva", "energy"), .combine = "cbind"  
    ...
    if (model == 3)    #circle with additive noise
    {
      thetas = 2*pi*runif(n);
      xs = 10*cos(thetas) + noise*rnorm(n); #xs = rnorm(n),ys = rnorm(n)独立
      ys = 10*sin(thetas) + noise*rnorm(n); #coef越小越相关 
      
    }
    
    
    if (model ==4)    # spiral circle with additive noise 
    {
      u = runif(n)*4;
      xs = 3*u*sin( u*pi ) + noise*rnorm(n); #xs = rnorm(n),ys = rnorm(n)独立
      ys = 3*u*cos( u*pi ) + noise*rnorm(n); #coef越小越相关 
    }
...
POWER_CALCULATIONd(3, 100 , sqrt(0.1), 25)
